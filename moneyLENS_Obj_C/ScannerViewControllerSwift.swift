//
//  ScannerViewControllerSwift.swift
//  moneyLENS_Obj_C
//
...
class ScannerViewControllerSwift : UIViewController  {
	...
	init(scanner aScanner: (MSScanner!), delegate: (MSAutoScannerSessionDelegate!)) {
		scannerSession = MSAutoScannerSession(scanner:aScanner)
		scannerSession.delegate = delegate
		scannerSession.resultTypes = MSResultTypeImage/// Todo: allow custom results (QR code, etc)
	...
	deinit {
		println("moodstocks: stopping scanner session")
